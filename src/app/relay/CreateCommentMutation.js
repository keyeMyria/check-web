...
class CreateCommentMutation extends Relay.Mutation {
  ...
  getFatQuery() {
    ...
    case 'project_source':
      query = Relay.QL`fragment on CreateCommentPayload { commentEdge, project_source { team_source { log, log_count } } }`;
      break;
    case 'project_media':
      query = Relay.QL`fragment on CreateCommentPayload { commentEdge, project_media { log, log_count } }`;
      break;
    case 'team_source':
      query = Relay.QL`fragment on CreateCommentPayload { commentEdge, team_source { log, log_count } }`;
      break;
    }
    return query;
  }
