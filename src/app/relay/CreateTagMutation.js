...
class CreateTagMutation extends Relay.Mutation {
  ...
  getFatQuery() {
    ...
    case 'source':
      query = Relay.QL`fragment on CreateTagPayload { tagEdge, source { id, dbid } }`;
      break;
    case 'project_media':
      query = Relay.QL`fragment on CreateTagPayload { tagEdge, project_media { log, tags, log_count } }`;
      break;
    case 'project_source':
      query = Relay.QL`fragment on CreateTagPayload { tagEdge, project_source { team_source { log, tags, log_count } } }`;
      break;
    }
    return query;
  }
